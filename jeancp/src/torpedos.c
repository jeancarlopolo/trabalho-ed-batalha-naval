        ...
        case 'c':
            ...
            if (sqrt(pow(x - x1, 2) + pow(y - y1, 2)) < raio)
            {
                ...
                fprintf(textow, "Torpedo destruiu: circulo \nid: %d \ncor da borda: %s \ncor do preenchimento: %s \nraio: %f \narea: %f \nX: %f \nY: %f \n", circulo_get_i(getInfo(barco)), circulo_get_corb(getInfo(barco)), circulo_get_corp(getInfo(barco)), circulo_get_r(getInfo(barco)), circulo_get_area(getInfo(barco)), circulo_get_x(getInfo(barco)), circulo_get_y(getInfo(barco)));
                    ...
                    fprintf(textow, "Torpedo atingiu: circulo \nid: %d \ncor da borda: %s \ncor do preenchimento: %s \nraio: %f \narea: %f \nX: %f \nY: %f \nHP: %d\n", circulo_get_i(getInfo(barco)), circulo_get_corb(getInfo(barco)), circulo_get_corp(getInfo(barco)), circulo_get_r(getInfo(barco)), circulo_get_area(getInfo(barco)), circulo_get_x(getInfo(barco)), circulo_get_y(getInfo(barco)), hp);
                }
            }
            break;
        case 'r':
            ...
            if (x >= x1 && x <= x1 + w && y >= y1 && y <= y1 + h)
            {
                ...
                {
                    destruido = true;
                    fprintf(textow, "Torpedo destruiu: retangulo \nid: %d \ncor da borda: %s \ncor do preenchimento: %s \naltura: %f \nlargura: %f \narea: %f \nX: %f \nY: %f \n", retangulo_get_i(getInfo(barco)), retangulo_get_corb(getInfo(barco)), retangulo_get_corp(getInfo(barco)), retangulo_get_h(getInfo(barco)), retangulo_get_w(getInfo(barco)), retangulo_get_area(getInfo(barco)), retangulo_get_x(getInfo(barco)), retangulo_get_y(getInfo(barco)));
                }else{
                    fprintf(textow, "Torpedo atingiu: retangulo \nid: %d \ncor da borda: %s \ncor do preenchimento: %s \naltura: %f \nlargura: %f \narea: %f \nX: %f \nY: %f \nHP: %d\n", retangulo_get_i(getInfo(barco)), retangulo_get_corb(getInfo(barco)), retangulo_get_corp(getInfo(barco)), retangulo_get_h(getInfo(barco)), retangulo_get_w(getInfo(barco)), retangulo_get_area(getInfo(barco)), retangulo_get_x(getInfo(barco)), retangulo_get_y(getInfo(barco)), hp);
                }
            }
            break;
        case 't':
            ...
            if (x == x1 && y == y1)
            {
                ...
                if (hp - 1 == 0)
                {
                    ...
                    fprintf(textow, "Torpedo atingiu: texto \nid: %d \ncor da borda: %s \ncor do preenchimento: %s \nX: %f \nY: %f \nHP: %d\n", texto_get_i(getInfo(barco)), texto_get_corb(getInfo(barco)), texto_get_corp(getInfo(barco)), texto_get_x(getInfo(barco)), texto_get_y(getInfo(barco)), hp);
                }
            }
            break;
        case 'l':
            ...
            if (((x <= x1 && x >= x2) || (x >= x1 && x <= x2)) && ((y >= y1 && y <= y2) || (y <= y1 && y >= y2)))
            {
                ...
                if (hp - 1 == 0)
                {
                    ...
                    fprintf(textow, "Torpedo atingiu: linha \nid: %d \ncor: %s \nX1: %f \nY1: %f \nX2: %f \nY2: %f \nHP: %d\n", linha_get_i(getInfo(barco)), linha_get_cor(getInfo(barco)), linha_get_x1(getInfo(barco)), linha_get_y1(getInfo(barco)), linha_get_x2(getInfo(barco)), linha_get_y2(getInfo(barco)), hp);
                }
            }
            break;
        default:
    ...
    while (elemento != NULL)
    {
        ...
        case 'c':
            ...
            if (sqrt(pow(x - x2, 2) + pow(y - y2, 2)) < raio)
            {
                ...
                insert(lista, circle);
                fprintf(textow, "Torpedo replicante: circulo \nid: %d \nraio: %f \ncor da borda: %s \ncor de preenchimento: %s \nX: %f \nY: %f \n", circulo_get_i(getInfo(barco)), circulo_get_r(getInfo(barco)), circulo_get_corb(getInfo(barco)), circulo_get_corp(getInfo(barco)), circulo_get_x(getInfo(barco)), circulo_get_y(getInfo(barco)));
                fprintf(textow, "Copia de: circulo \nid: %d \nraio: %f \ncor da borda: %s \ncor de preenchimento: %s \nX: %f \nY: %f \n", circulo_get_i(getInfo(circle)), circulo_get_r(getInfo(circle)), circulo_get_corb(getInfo(circle)), circulo_get_corp(getInfo(circle)), circulo_get_x(getInfo(circle)), circulo_get_y(getInfo(circle)));
            }
            break;
        case 'r':
            ...
            if (x >= x2 && x <= x2 + w && y >= y2 && y <= y2 + h)
            {
                ...
                fprintf(textow, "Torpedo replicante: retangulo \nid: %d \naltura: %f \nlargura: %f \ncor da borda: %s \ncor de preenchimento: %s \nX: %f \nY: %f \n", retangulo_get_i(getInfo(barco)), retangulo_get_h(getInfo(barco)), retangulo_get_w(getInfo(barco)), retangulo_get_corb(getInfo(barco)), retangulo_get_corp(getInfo(barco)), retangulo_get_x(getInfo(barco)), retangulo_get_y(getInfo(barco)));
                fprintf(textow, "Copia de: retangulo \nid: %d \naltura: %f \nlargura: %f \ncor da borda: %s \ncor de preenchimento: %s \nX: %f \nY: %f \n", retangulo_get_i(getInfo(rectangle)), retangulo_get_h(getInfo(rectangle)), retangulo_get_w(getInfo(rectangle)), retangulo_get_corb(getInfo(rectangle)), retangulo_get_corp(getInfo(rectangle)), retangulo_get_x(getInfo(rectangle)), retangulo_get_y(getInfo(rectangle)));
            }
            break;
        case 't':
            ...
            if (x == x2 && y == y2)
            {
                ...
                insert(lista, text);
                fprintf(textow, "Torpedo replicante: texto \nid: %d \nconteudo: %s \ncor da borda: %s \ncor de preenchimento: %s \nX: %f \nY: %f \n", texto_get_i(getInfo(barco)), texto_get_conteudo(getInfo(barco)), texto_get_corb(getInfo(barco)), texto_get_corp(getInfo(barco)), texto_get_x(getInfo(barco)), texto_get_y(getInfo(barco)));
                fprintf(textow, "Copia de: texto \nid: %d \nconteudo: %s \ncor da borda: %s \ncor de preenchimento: %s \nX: %f \nY: %f \n", texto_get_i(getInfo(text)), texto_get_conteudo(getInfo(text)), texto_get_corb(getInfo(text)), texto_get_corp(getInfo(text)), texto_get_x(getInfo(text)), texto_get_y(getInfo(text)));
            }
            break;
        case 'l':
            ...
            if (((x <= x2 && x >= x3) || (x >= x2 && x <= x3)) && ((y >= y2 && y <= y3) || (y <= y2 && y >= y3)))
            {
                ...
                fprintf(textow, "Copia de: linha \nid: %d \ncor: %s \nX1: %f \nY1: %f \nX2: %f \nY2: %f \n", linha_get_i(getInfo(line)), linha_get_cor(getInfo(line)), linha_get_x1(getInfo(line)), linha_get_y1(getInfo(line)), linha_get_x2(getInfo(line)), linha_get_y2(getInfo(line)));
            }
            break;
        default:
            break;
